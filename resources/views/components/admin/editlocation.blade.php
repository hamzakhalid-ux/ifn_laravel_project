<div class="">
    <div class="card card-info">
        ...
        <form class="form-horizontal" id=""  method="POST" action="{{ url('admin/location/update-location') }}">
            @csrf
            <input type="hidden" name="location[loc_id]" value="{{$location->loc_id ?? ''}}">
            <div class="card-body row">
                <div class="col-sm-6">
                    <div class="form-group row">
                        <label for="cat_name" class="col-sm-3 col-form-label">Location Name</label>
                            <div class="col-sm-9">
                                <input type="text" name="location[location_title]" required value="{{$location->title ?? ''}}" class="form-control"  placeholder="Title Name">
                            </div>
                    </div>
                </div>
                <div class="col-sm-6">
                        ...
                        <select class="form-control js-example-basic-multiple" id=""  name="location[location_parent_id]" >
                            ...
                            @if(!empty($locations))
                                @foreach($locations as $loc)
                                    <option @if(!empty($location) && $location->parent_id == $loc['loc_id'] ) selected @endif value="{{$loc['loc_id']}}">{{$loc['title'] .(!empty($loc['parent_title']) ? "(".$loc['parent_title'].")" : '' )}}</option>
                                @endforeach
                            @endif
                        </select>
                    </div>
